...
import Lucee.Hash (computeHashesForExtensions, computeHashesForVersion)
import Lucee.Nix (generateDefinitions, generateExtensionDefinition, nixExtensionsFileHeader, nixFileFooter)
import Lucee.Parse (parseExtensions, parseVersions)
import Lucee.Types
...
updateDefinitions config = do
  ...
  versionResults <-
    liftIO $
      mapConcurrently
        ( \version ->
            runExceptT (computeHashesForVersion version)
        )
        filteredVersions
  ...
  extensions <-
    if configIncludeExtensions config
